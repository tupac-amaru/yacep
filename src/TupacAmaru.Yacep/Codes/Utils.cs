...
namespace TupacAmaru.Yacep.Codes
{
    internal static class Utils
    {
        ...
        internal static bool IsDigit1(this object x) => x is char || x is sbyte || x is byte ||
                                                       x is short || x is ushort || x is int || x is uint ||
                                                       x is long || x is ulong || x is float || x is double || x is decimal;
        internal static bool IsDigit2(this object x) => x is char || x is sbyte || x is byte ||
                                                       x is short || x is ushort || x is int || x is uint ||
                                                       x is long || x is ulong || x is float || x is double || x is decimal;
